      ...
      <Link to={`/news/${newsItem.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
        <SpotlightCard className="h-100 d-flex flex-column" spotlightColor="rgba(250, 204, 21, 0.15)">
          ...
          <div className="card-body p-4 d-flex flex-column flex-grow-1">
            ...
            <h4 className="card-title fw-bold mb-3 text-white" style={{ minHeight: '60px' }}>{newsItem.title}</h4>
...
export default function Home() {
  ...
  useEffect(() => {
    // News
    axios.get(`${API_HOME}/api/news`)
      .then((res) => setNews(res.data))
      .catch((err) => console.error(err));

    // Team
    axios.get(`${API_HOME}/api/team?home=true`)
      .then((res) => {
        const teamData = res.data;
        setTeam(teamData);
        setFilteredTeam(teamData); 

        // --- PERBAIKAN LOGIKA DIVISI ---
        // Karena di database kolom 'division' NULL, kita ambil dari 'position'
        const allPositions = teamData.map(member => member.position);
        
        // Filter: Hapus yang kosong/null, lalu ambil unik
  ...
  // 3. Logic Filter (Berdasarkan Position)
  ...
  return (
    <div style={{ overflowX: 'hidden' }}>
      ...
      <header className="position-relative text-white text-center" style={{ background: 'transparent', minHeight: '95vh', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingTop: '60px', overflow: 'visible' }}>
        <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 6, repeat: Infinity }} className="position-absolute top-50 start-50 translate-middle bg-primary rounded-circle" style={{ width: '600px', height: '600px', filter: 'blur(180px)', opacity: 0.2, zIndex: 0 }} />
        <motion.div animate={{ x: [-20, 20, -20], y: [-20, 20, -20] }} transition={{ duration: 8, repeat: Infinity }} className="position-absolute top-0 end-0 bg-warning rounded-circle" style={{ width: '400px', height: '400px', filter: 'blur(150px)', opacity: 0.15, zIndex: 0 }} />
            ...
            <div className="col-lg-10">
              <motion.div initial="hidden" animate="visible" variants={fadeInUp}>
                ...
                <div className="d-flex gap-3 justify-content-center flex-wrap">
                  <Link to="/join" className="btn btn-warning btn-lg px-5 py-3 rounded-pill fw-bold shadow-lg hover-scale text-dark border-0">JOIN THE TEAM</Link>
                  <Link to="/achievement" className="btn btn-outline-light btn-lg px-5 py-3 rounded-pill fw-bold hover-scale backdrop-blur">VIEW TRACK RECORD</Link>
                </div>
              </motion.div>
            </div>
          </div>
      ...
      <div className="bg-white py-5 shadow-sm position-relative" style={{ marginTop: '-40px', zIndex: 10, borderRadius: '40px 40px 0 0' }}>
        <div className="container">
          <div className="row text-center g-4">
            {[{ label: 'Years of Experience', val: 12, suffix: '+' }, { label: 'Awards Won', val: 20, suffix: '+' }, { label: 'Team Members', val: 50, suffix: '+' }, { label: 'Battery Efficiency', val: 98, suffix: '%' }].map((stat, idx) => (
              <div className="col-md-3 col-6" key={idx}>
                <h2 className="fw-bold display-4 text-elviro-blue mb-0"><CountUp end={stat.val} duration={3} enableScrollSpy />{stat.suffix}</h2>
        ...
        <div className="container py-5">
           <div className="row justify-content-center text-center">
            <div className="col-lg-9" data-aos="fade-up">
              ...
              <h2 className="display-4 fw-bold text-dark mb-5">EEPIS Electric Vehicle <br /> Prototype Concept</h2>
              <div className="bg-white p-5 rounded-5 shadow-sm border-top border-5 border-warning position-relative overflow-hidden">
                ...
                <p className="text-dark lead mb-4 position-relative z-1">Formerly known as CHAPENS Team, ELVIRO is a research team from <strong>Politeknik Elektronika Negeri Surabaya (PENS)</strong> established in 2023.</p>
                <div className="row justify-content-center mt-4 position-relative z-1">
                  <div className="col-md-5 mb-4 mb-md-0"><div className="p-3 rounded-4 bg-light h-100"><i className="bi bi-battery-charging fs-1 text-primary mb-2"></i><h5 className="fw-bold mb-1">Battery Powered</h5></div></div>
                  <div className="col-md-5"><div className="p-3 rounded-4 bg-light h-100"><i className="bi bi-speedometer2 fs-1 text-primary mb-2"></i><h5 className="fw-bold mb-1">High Efficiency</h5></div></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-5 my-5">
        <div className="container">
          ...
          <div className="row g-4 justify-content-center">
            {news.length > 0 ? news.slice(0, 3).map((item, idx) => <NewsCard key={item.id} newsItem={item} index={idx} />) : <div className="col-12 text-center py-5"><p className="text-muted">Belum ada berita terbaru.</p></div>}
          </div>
        </div>
      </section>

      {/* 5. TEAM SECTION (TECH UI + DROPDOWN FIXED) */}
      <section className="py-5 text-white position-relative" style={{ background: '#0b1120', minHeight: '800px', overflow: 'visible' }}>
        {/* Decorative BG Elements */}
        <div className="position-absolute start-0 top-0 w-100 h-100" style={{ backgroundImage: 'linear-gradient(rgba(255, 255, 255, 0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(255, 255, 255, 0.03) 1px, transparent 1px)', backgroundSize: '40px 40px', pointerEvents:'none' }}></div>
        
        <div className="container py-5 position-relative z-1">
          {/* HEADER & DROPDOWN - Z-INDEX TINGGI (1000) */}
          <div className="d-flex flex-column align-items-center mb-5 position-relative" style={{ zIndex: 1000 }}>
            <h2 className="fw-bold display-5 mb-3">Meet The <span className="text-warning">Squad</span></h2>
            
            {/* --- CUSTOM DROPDOWN FILTER (FIXED) --- */}
            <div className="position-relative mt-2" ref={dropdownRef} style={{ width: '250px' }}>
              ...
              >
                <span className="fw-medium text-uppercase ls-1 small text-truncate">
                  {selectedDivision === 'All Divisions' ? 'FILTER: ALL TEAMS' : selectedDivision}
                </span>
              ...
              <AnimatePresence>
                {isDropdownOpen && (
                  <motion.div 
                    ...
                    // Z-index SANGAT TINGGI agar di atas card
                    style={{ background: '#1e293b', zIndex: 9999 }} 
                  >
                    <div className="d-flex flex-column" style={{ maxHeight: '300px', overflowY: 'auto' }}>
                      <button 
                        className={`btn text-start text-white py-2 px-3 border-0 rounded-0 tech-dropdown-item ${selectedDivision === 'All Divisions' ? 'bg-warning text-dark' : ''}`}
                        onClick={() => handleFilterSelect('All Divisions')}
                      >
                        All Divisions
                      </button>
                      ...
                      {divisions.length > 0 ? divisions.map((div, idx) => (
                        <button 
                          key={idx}
                          className={`btn text-start text-white py-2 px-3 border-0 rounded-0 tech-dropdown-item ${selectedDivision === div ? 'bg-warning text-dark' : ''}`}
                          onClick={() => handleFilterSelect(div)}
                        >
                          {div}
                        </button>
                      )) : (
          ...
          {/* --- GRID TEAM (TECH CARD UI - KEMBALI KE YANG DARK) --- */}
          <motion.div 
            layout 
            className="row row-cols-2 row-cols-md-3 row-cols-xl-5 g-4 justify-content-center" 
            style={{ position: 'relative', zIndex: 1 }} // Z-index rendah
          >
                  ...
                  >
                    ...
                    <div className="card h-100 border-0 rounded-4 overflow-hidden tech-card bg-dark text-white position-relative">
                      ...
                      <div className="position-relative" style={{ height: '240px' }}>
                        <img src={`${API_HOME}/uploads/team/${t.photo}`} alt={t.name} className="w-100 h-100 object-fit-cover tech-img" />
                      ...
                      <div className="card-body p-3 pt-0 position-relative z-1" style={{ backgroundColor: '#1e293b' }}>
                         <h6 className="fw-bold text-white mb-1 text-truncate font-heading" style={{ fontSize: '1rem' }} title={t.name}>{t.name}</h6>
                         <p className="small text-white-50 text-uppercase ls-1 mb-3" style={{ fontSize: '0.7rem' }}>{t.position}</p>
                         ...
                         <div className="row g-1">
                           <div className="col-12">
                             <div className="bg-black bg-opacity-40 p-2 rounded-2 border border-secondary border-opacity-25 d-flex align-items-center gap-2">
                               ...
                               <span className="text-truncate text-white-50 small" style={{fontSize: '0.7rem'}}>{t.major}</span>
                             </div>
                           </div>
                           <div className="col-6">
                             <div className="bg-black bg-opacity-40 p-1 rounded-2 border border-secondary border-opacity-25 text-center">
                               <small className="d-block text-secondary" style={{fontSize: '0.6rem'}}>NRP</small>
                               <span className="text-white small fw-bold" style={{fontSize: '0.7rem'}}>{t.nrp}</span>
                             </div>
                           </div>
                           <div className="col-6">
                             <div className="bg-black bg-opacity-40 p-1 rounded-2 border border-secondary border-opacity-25 text-center">
                               <small className="d-block text-secondary" style={{fontSize: '0.6rem'}}>BATCH</small>
                               <span className="text-warning small fw-bold" style={{fontSize: '0.7rem'}}>{t.batch}</span>
                             </div>
                           </div>
                         </div>
                      </div>
      ...
      <section className="py-5 text-center position-relative overflow-hidden">
        ...
        <div className="container position-relative z-2 py-5" data-aos="fade-up">
          ...
          <Link to="/join" className="btn btn-dark btn-lg px-5 py-3 rounded-pill fw-bold shadow-lg hover-scale border-0">REGISTER NOW</Link>
        </div>
      </section>
